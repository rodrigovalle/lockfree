...
void test_wraparound() {
    ...
    spscq_push(queue, 0);
    spscq_push(queue, 1);
    spscq_pop(queue, &disposal);
    spscq_pop(queue, &disposal);

    // fill the queue
    for (int64_t i = 0; i < 16; i++) {
        assert(spscq_push(queue, i));
    }
    assert(!spscq_push(queue, 0));

    // empty the queue
    for (int64_t i = 0; i < 16; i++) {
        int64_t val;
        assert(spscq_pop(queue, &val));
        assert(val == i);
    }
    assert(!spscq_pop(queue, &disposal));
    assert(spscq_push(queue, 0));
}
